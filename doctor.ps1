<#
.SYNOPSIS
  Doctor - one-command Armory environment health check.
#>
...
function Show-HelpText {
    ...
    Write-Host "    .\\doctor.ps1 -Detailed"
    Write-Host "    .\\doctor.ps1 -Check config,wrapper"
    Write-Host "    .\\doctor.ps1 -Output \"$env:USERPROFILE\\.armory\\reports\\doctor.txt\""
    Write-Host ""
}
    ...
    switch ($c) {
        ...
        "ci" {
            ...
            foreach ($rel in $requiredCiFiles) {
                if (-not (Test-Path (Join-Path $PSScriptRoot $rel))) {
                    $missingCi += $rel
                }
            }
...
$summaryLines = @()
$summaryLines += "Doctor Environment Check"
$summaryLines += "------------------------"
...
if ($Output) {
    ...
    Write-Host "Doctor report written: $outPath" -ForegroundColor Green
} else {
